...
`;class tM extends HTMLElement{#$=null;#I=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tR.content.cloneNode(!0))}async connectedCallback(){this.#$=this.shadowRoot?.getElementById("historyList"),this.#I=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#B((await X())[1]||[]),this.#$?.addEventListener("click",this.#D),this.#I?.addEventListener("click",this.#F)}disconnectedCallback(){this.#$?.removeEventListener("click",this.#D),this.#I?.removeEventListener("click",this.#F)}async add(t){let[,e]=await K();if(!t||!e?.addToHistory)return;let[o,i=[]]=await X();if(!o&&Array.isArray(i)&&!i.find(e=>e===t)){let e=[...i,t],[o]=await Q(e);o||this.#$.appendChild(this.#P(t))}}async remove(t){if(!t)return;let[e,o=[]]=await X();if(!e&&Array.isArray(o)){let e=o.filter(e=>e!==t),[i]=await Q(e);if(!i){let e=this.#$.querySelector(`li[data-value="${t}"]`);e&&e.remove()}}}async empty(){let[t]=await Q([]);t||this.#$.replaceChildren()}#B(t){if(!this.#$)return;this.#$.replaceChildren();let e=document.createDocumentFragment();t.forEach(t=>{e.appendChild(this.#P(t))}),this.#$.appendChild(e)}#P(t){let e;let o=window.crypto&&"function"==typeof window.crypto.randomUUID?window.crypto.randomUUID():Array.from({length:5},()=>Math.random().toString(36).substring(2,8)).join("-"),i=document.createElement("li");i.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t,e.setAttribute("id",`historyItem-${o}`);let r=document.createElement("div");r.className="actions";let s=document.createElement("custom-clipboard-copy");s.setAttribute("id",`copyHistoryItem-${o}`),s.setAttribute("aria-label","Copy to clipboard"),s.setAttribute("aria-labelledby",`copyHistoryItem-${o} historyItem-${o}`),s.setAttribute("only-icon",""),s.setAttribute("value",t),r.appendChild(s);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("id",`removeHistoryItem-${o}`),a.setAttribute("aria-label","Remove from history"),a.setAttribute("aria-labelledby",`removeHistoryItem-${o} historyItem-${o}`),a.innerHTML=`
