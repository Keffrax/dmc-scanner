...
`,tr=document.createElement("style");function ts(t){t&&(t.querySelector("bs-result")?.remove(),t.querySelectorAll(".model-info").forEach(t=>t.remove()))}async function ta(t,e){if(!t||!e)return;ts(t);let o=to(e),i=document.createElement("bs-result");i.setAttribute("value",e),i.setAttribute("model",o),i.setAttribute("role","alert"),i.setAttribute("aria-live","assertive"),i.setAttribute("aria-atomic","true"),t.appendChild(i);let r=document.createElement("div");r.classList.add("model-info-container"),o.split("\n").filter(t=>""!==t.trim()).forEach(t=>{let e=document.createElement("div");e.classList.add("model-info"),t.includes("Neznámy model")&&e.classList.add("unknown"),e.textContent=t,r.appendChild(e)}),t.appendChild(r)}tr.type="text/css",tr.innerText=ti,document.head.appendChild(tr);let tn=(()=>{let t=new(window.AudioContext||window.webkitAudioContext||window.audioContext);if(t)return e=>{let{duration:o,frequency:i,volume:r,type:s,onEnded:a}=e,n=t.createOscillator(),l=t.createGain();n.connect(l),l.connect(t.destination),r&&(l.gain.value=r),i&&(n.frequency.value=i),s&&(n.type=s),"function"==typeof a&&(n.onended=a),n.start(t.currentTime),n.stop(t.currentTime+(o||500)/1e3)}})();async function tl(t=0){if("function"==typeof window.navigator.vibrate)try{window.navigator.vibrate(t)}catch{}}async function td(t={}){let{success:e=!0}=t,[,o]=await K();o&&(o.beep&&tn(e?{duration:200,frequency:860,volume:.03,type:"square"}:{duration:300,frequency:200,volume:.05,type:"sawtooth"}),o.vibrate&&tl(e?100:200))}function tc(t,e){if(!t||!e)return;let o=t.getBoundingClientRect();e.style.cssText=`width: ${o.width}px; height: ${o.height}px`}let th=["aztec","code_128","code_39","code_93","codabar","data_matrix","ean_13","ean_8","itf","pdf417","qr_code","upc_a","upc_e"];class tu{static async polyfill(){if(!("BarcodeDetector"in window))try{await s("bkcP3")}catch(t){throw Error("BarcodeDetector API is not supported by your browser.",{cause:t})}}static async getSupportedFormats(){let t=await window.BarcodeDetector.getSupportedFormats()||[];return th.filter(e=>t.includes(e))}static async create(t){let e=Array.isArray(t)&&t.length>0?t:await tu.getSupportedFormats();return new tu(e)}static async setup(){try{return await tu.polyfill(),{barcodeReaderError:null}}catch(t){return{barcodeReaderError:t}}}constructor(t){this.barcodeReader=new window.BarcodeDetector({formats:t})}async detect(t){if(!this.barcodeReader)throw Error("BarcodeReader is not initialized.");let e=await this.barcodeReader.detect(t);if(Array.isArray(e)&&e.length>0){let t=e[0];return t.rawValue,t.format,t}throw Error("Could not detect barcode from provided source.")}}function tm(t={}){let{el:e,isTorchOn:o}={el:document.getElementById("torchButton"),isTorchOn:!1,...t},i=e.querySelectorAll("svg path");2===i.length&&(i[0].style.display=o?"none":"block",i[1].style.display=o?"block":"none",e.setAttribute("aria-label",`Turn ${o?"off":"on"} flash`))}let tp="video-capture",tb=`
 ...
 * @author George Raptis <georapbox@gmail.com>
 * @license MIT
 */function tC(t){return null!==t&&"object"==typeof t?"share"in navigator&&"canShare"in navigator&&navigator.canShare(t):"share"in navigator}tk.defineCustomElement();let tL=`
  :host {
    box-sizing: border-box;
  }
...
`;class tM extends HTMLElement{#H=null;#I=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tR.content.cloneNode(!0))}async connectedCallback(){this.#H=this.shadowRoot?.getElementById("historyList"),this.#I=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#B((await X())[1]||[]),this.#H?.addEventListener("click",this.#D),this.#I?.addEventListener("click",this.#F)}disconnectedCallback(){this.#H?.removeEventListener("click",this.#D),this.#I?.removeEventListener("click",this.#F)}async add(t){let[,e]=await K();if(!t||!e?.addToHistory)return;let[o,i=[]]=await X();if(!o&&Array.isArray(i)&&!i.find(e=>e===t)){let e=[...i,t],[o]=await Q(e);o||this.#H.appendChild(this.#P(t))}}async remove(t){if(!t)return;let[e,o=[]]=await X();if(!e&&Array.isArray(o)){let e=o.filter(e=>e!==t),[i]=await Q(e);if(!i){let e=this.#H.querySelector(`li[data-value="${t}"]`);e&&e.remove()}}}async empty(){let[t]=await Q([]);t||this.#H.replaceChildren()}#B(t){if(!this.#H)return;this.#H.replaceChildren();let e=document.createDocumentFragment();t.forEach(t=>{e.appendChild(this.#P(t))}),this.#H.appendChild(e)}#P(t){let e;let o=window.crypto&&"function"==typeof window.crypto.randomUUID?window.crypto.randomUUID():Array.from({length:5},()=>Math.random().toString(36).substring(2,8)).join("-"),i=document.createElement("li");i.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t,e.setAttribute("id",`historyItem-${o}`);let r=document.createElement("div");r.className="actions";let s=document.createElement("custom-clipboard-copy");s.setAttribute("id",`copyHistoryItem-${o}`),s.setAttribute("aria-label","Copy to clipboard"),s.setAttribute("aria-labelledby",`copyHistoryItem-${o} historyItem-${o}`),s.setAttribute("only-icon",""),s.setAttribute("value",t),r.appendChild(s);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("id",`removeHistoryItem-${o}`),a.setAttribute("aria-label","Remove from history"),a.setAttribute("aria-labelledby",`removeHistoryItem-${o} historyItem-${o}`),a.innerHTML=`
    ...
    `,r.appendChild(a),i.appendChild(e),i.appendChild(r),i}#D=t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;window.confirm(`Delete history item ${t}?`)&&this.remove(t)}};#F=()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};static defineCustomElement(t="bs-history"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tM)}}tM.defineCustomElement(),async function(){let t;let e=document.querySelector("a-tab-group"),o=document.querySelector("video-capture"),i=document.querySelector("bs-settings"),r=document.querySelector("bs-history"),s=document.getElementById("cameraPanel"),a=document.getElementById("filePanel"),n=document.getElementById("scanInstructions"),l=document.getElementById("scanBtn"),d=document.getElementById("dropzone"),c=document.querySelector("resize-observer"),h=document.getElementById("scanFrame"),u=document.getElementById("torchButton"),m=document.getElementById("globalActions"),p=document.getElementById("historyBtn"),b=document.getElementById("historyDialog"),g=document.getElementById("settingsBtn"),f=document.getElementById("settingsDialog"),v=document.getElementById("settingsForm"),y=document.getElementById("cameraSelect"),w=!0;"function"==typeof HTMLDialogElement&&(m?.removeAttribute("hidden"),b?.removeAttribute("hidden"),f?.removeAttribute("hidden"));let{barcodeReaderError:E}=await tu.setup();if(E){let t=document.getElementById("barcodeReaderError");w=!1,m?.setAttribute("hidden",""),e?.setAttribute("hidden",""),t?.removeAttribute("hidden"),t.textContent=E?.message;return}let A=await tu.getSupportedFormats(),[,x]=await K(),k=x?.formats||A,C=await tu.create(k);o.addEventListener("video-capture:video-play",_,{once:!0}),o.addEventListener("video-capture:error",function(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;s.innerHTML=`<div class="alert alert-danger" role="alert" style="margin: 0;">${o}</div>`},{once:!0}),tf.defineCustomElement();let L=o?.shadowRoot,S=L?.querySelector("video"),z=L?.querySelector('[part="actions-container"]');async function T(){console.log("Scanning..."),n?.removeAttribute("hidden");try{let e=await C.detect(S),o=e?.rawValue??"";if(!o)throw Error(F);window.cancelAnimationFrame(t),ts(s),ta(s,`Obsah k\xf3du: ${o}`);let i=to(o);ta(s,i),r?.add(o),n?.setAttribute("hidden",""),l?.removeAttribute("hidden"),h?.setAttribute("hidden",""),z?.setAttribute("hidden",""),td();return}catch{}w&&(t=window.requestAnimationFrame(()=>T()))}async function _(t){h?.removeAttribute("hidden"),tc(t.detail.video,h),T();let e=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(u?.removeAttribute("hidden"),o.hasAttribute("torch")&&tm({el:u,isTorchOn:!0})),e?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=e?.zoom||1;t?.removeAttribute("hidden"),r.textContent=n,t.addEventListener("click",t=>{let e=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');e&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n,o.zoom=n})}let s=await tf.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,y.appendChild(o)}),s.length>1&&y?.removeAttribute("hidden")}async function R(t){t.preventDefault();let e={},o=new FormData(v),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,J(e),"formats-settings"===t.target.name&&(C=await tu.create(r))}d.accept=P.join(","),i.supportedFormats=A,l.addEventListener("click",function(){l?.setAttribute("hidden",""),h?.removeAttribute("hidden"),z?.removeAttribute("hidden"),ts(s),T()}),e.addEventListener("a-tab-show",tt(function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(w=!0,!o)return;let t=null!=s.querySelector("bs-result");if(o.loading||t||(h?.removeAttribute("hidden"),z?.removeAttribute("hidden"),T()),"function"==typeof o.startVideoStream){let t=y?.value||void 0;o.startVideoStream(t)}}else"fileTab"===e&&(w=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream(),h?.setAttribute("hidden",""),z?.setAttribute("hidden",""))},250)),d.addEventListener("files-dropzone-drop",function(t){!function(t){if(!t)return;let e=new Image,o=new FileReader;o.onload=o=>{let i=o.target.result;e.onload=async()=>{try{let t=await C.detect(e),o=t?.rawValue??"";if(!o)throw Error(F);ta(a,o),r?.add(o),td()}catch(t){ta(a,F),td({success:!1})}},e.src=i,e.alt="Image preview",d.replaceChildren();let s=document.createElement("div");s.className="dropzone-preview",s.setAttribute("aria-hidden","true");let n=document.createElement("div");n.className="dropzone-preview__image-wrapper";let l=document.createElement("div");l.className="dropzone-preview__file-name",l.textContent=t.name,n.appendChild(e),s.appendChild(n),s.appendChild(l),d.prepend(s)},o.readAsDataURL(t)}(t.detail.acceptedFiles[0])}),c.addEventListener("resize-observer:resize",function(){tc(o.shadowRoot.querySelector("video"),h)}),g.addEventListener("click",function(){f.open=!0}),v.addEventListener("change",tt(R,500)),p.addEventListener("click",function(){b.open=!0}),u.addEventListener("click",function(t){o.torch=!o.torch,tm({el:t.currentTarget,isTorchOn:o.hasAttribute("torch")})}),y.addEventListener("change",function(t){if("function"!=typeof o.restartVideoStream)return;let e=t.target.value||void 0;o.restartVideoStream(e)}),document.addEventListener("visibilitychange",function(){if("cameraTab"===e.querySelector("[selected]").getAttribute("id")){if("hidden"===document.visibilityState)w=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream();else{w=!0;let t=document.querySelector("video-capture");if(!t)return;if(t.loading||s.querySelector("bs-result")||T(),"function"==typeof t.startVideoStream){let e=y?.value||void 0;t.startVideoStream(e)}}}}),document.addEventListener("keydown",function(t){"Escape"===t.key&&function(){let t=e.querySelector("#cameraTab").hasAttribute("selected"),o=!l.hidden,i=f.hasAttribute("open"),r=b.hasAttribute("open");o&&t&&!(i||r)&&l.click()}()})}()}();
//# sourceMappingURL=index.c5c2b3e2.js.map
